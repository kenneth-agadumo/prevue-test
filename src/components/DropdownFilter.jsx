...
const DropdownFilter = ({ filterHandler }) => {
  const [isOpen, setIsOpen] = useState(false); // Toggle dropdown visibility
  const [selectedOption, setSelectedOption] = useState("All"); // Track the selected option

  const options = ["All", "Restaurants", "Shortlets"]; // Dropdown options
  ...
  const handleOptionClick = (option) => {
    ...
    filterHandler(option); // Pass selected option to parent for filtering
  };
